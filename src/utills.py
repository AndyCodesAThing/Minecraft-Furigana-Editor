...
# #### Make files ####
def write_json_fromat(data, file_path, encoding=None, ensure_ascii=True):
...
def write_json_fromat_japanese(data, file_path):
    ...
    write_json_fromat(data=data, file_path=file_path, encoding="utf-8", ensure_ascii=False)
...
def mix_dicts(kanji, furigana, precentage_from_kanji):
    ...
    num_from_master = max(1, int(len(common_keys) * precentage_from_kanji / 100))
